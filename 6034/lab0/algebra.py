...
def is_multiplication_by_one(expr):
    return isinstance(expr, Product) and expr == [1]
...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    '*' will not help you.
    """
    if is_multiplication_by_one(expr1):
        return expr2
    if is_multiplication_by_one(expr2):
        return expr1

    if isinstance(expr1, Product) and isinstance(expr2, Product):
        return Product(expr1 + expr2).flatten()

    sum_expr = expr1 if isinstance(expr1, Sum) else expr2
    product_expr = expr1 if isinstance(expr1, Product) else expr2

    return Sum([Product([product_expr, expr]).flatten() for expr in sum_expr])
