...
```python
def sort_by_profit(ss, es, ls):
    rs = range(len(ss))
    xs = sorted(map(lambda x: (es[x] - ss[x], x), rs), reverse=True)
    return (
        [ss[x] for _, x in xs],
        [es[x] for _, x in xs],
        [ls[x] for _, x in xs],
...
# o(c^2t)
def stock_limited(total, count, start, end, limit):
    profit = {}
    purchase = {}

    # o(clogc)
    start, end, limit = map(
        lambda xs: [0] + xs, sort_by_profit(start, end, limit)
    )

    # o(c^2t)
    for c in range(total + 1):
        profit[c, 0] = c
        purchase[c, 0] = [0] * (count + 1)

        # o(c)
        for s in range(1, count + 1):
            profit[c, s] = profit[c, s - 1]
            purchase[c, s] = purchase[c, s - 1]
            ...
            if start[s] <= c:
                ...
                if pur[s] < limit[s]:
                    if profit[c, s] < end[s] + profit[lo, s]:
                        profit[c, s] = end[s] + profit[lo, s]
                        # o(c)
                        pur = pur[:]
                        pur[s] += 1
                        purchase[c, s] = pur
                else:
                    if profit[c, s] < end[s] + profit[lo, s - 1]:
                        profit[c, s] = end[s] + profit[lo, s - 1]
                        # o(c)
                        pur = purchase[lo, s - 1][:]
                        pur[s] += 1
                        purchase[c, s] = pur
...
profit[c, s] = max{
  profit[c, s - 1],
  profit[c - start[s], s] + end[s], # if limit not exceeded for stock s
  profit[c - start[s], s - 1] + end[s], # if limit exceeded for stock s
}

this recurrence solves the problem since we take the best option as long as we can and then consider next best option until we have solved the problem for all keys [c, s]. note that it is important that stocks are sorted by their return descending before we start filling in dp tables.
