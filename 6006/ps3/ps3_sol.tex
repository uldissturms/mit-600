...
Solution: LCA pseudo code starts at root node and goes left if the key is greater than l and right otherwise until (key >= l && key <= h) or bottom of the tree is reached.
Proof: If key is less than l then LCA must be in the right subtree, if key is greater than h then it must be in left subtree if it exists by the definition of BST.
       If key is greater or equal to l and less than or equal to h and we go left or right we miss part of the valid range.
