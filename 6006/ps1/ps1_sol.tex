...
will have dimensions
$m \times \lfloor n / 2 \rfloor$ or 
  $m \times \left(n - \lfloor n / 2 \rfloor - 1 \right)$ if sliced by column or $\lfloor m / 2 \rfloor \times n$ or $\left(m - \lfloor m / 2 \rfloor - 1 \right \times n$ if sliced by row.
...
Just prior to this,
it must examine a subproblem of size
$m \times 1$ or $m \times 2$ or $n \times 1$ or $n \times 2$.
If the problem is of size $m \times 1$ or $n \times 1$,
then calculating the maximum of the central column or row
is equivalent to calculating the maximum of the entire problem.
Hence, the maximum that the algorithm finds must be a peak,
and it will halt and return the location.
If the problem has dimensions $m \times 2$ or $n \times 2$,
then there are two possibilities:
either the maximum of the central column or row is a peak
(in which case the algorithm will halt and return the location),
or it has a strictly better neighbor in the other column or row
(in which case the algorithm will recurse
on the non-empty subproblem with dimensions $m \times 1$ or $n \times 1$,
thus reducing to the previous case).
...
it must eventually reach a level where it stops being a peak.
At that level, the location $(r_1, c_1)$
must be adjacent to the dividing column $c_2$ or row $r_2$ (where $|c_1 - c_2| = 1$ or $|r_1 - r_2| = 1$),
and the values must satisfy the inequality
$val(r_1, c_1) < val(r_1, c_2)$ or $val(r_1, c_1) < val(r_2, c_1)$ accordingly.
...
Hence, we have the following chain of inequalities:
$$val(r_1, c_1) < val(r_1, c_2) \le val(r_2, c_2) < val(r_2, c_1)$$
But in order for \texttt{algorithm4} to return $(r_1, c_1)$ as a peak,
the value at $(r_1, c_1)$ must have been the greatest in its column,
making $val(r_1, c_1) \ge val(r_2, c_1)$.
Hence, we have a contradiction. Equivalent proof can be made when sliting on a row.
\end{quote}
